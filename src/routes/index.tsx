...
const dailyBrief = {
  ...
  thesis:
    "The market is misreading falling token-price charts as an AI demand collapse. The stronger signal is a move from subsidized frontier usage toward broader, cheaper, more efficient AI adoption while infrastructure demand keeps expanding.",
  takeaways: [
    "Token price declines are not the same as token demand declines.",
    "Goldman argues AI infrastructure spending estimates remain too conservative.",
    "KKR, NVIDIA, and Kuwait's $10B data center company shows infrastructure is becoming a coordinated capital, power, and chip supply problem.",
    "Google looking beyond TSMC for TPU components reinforces how tight AI chip capacity remains.",
    "AI Daily Brief source links should become reusable inputs for Top Stories, AI Insider, VC Signals, and Geopolitics.",
  ],
  stories: [
    ["AI Insider", "The Silicon Data token chart measures average price per million tokens through routers, not total AI usage."],
    ["Infrastructure", "AI data center spending, power, packaging, and chip capacity are still the dominant bottlenecks."],
    ["VC Signals", "Full-stack infrastructure vehicles are emerging as a way to finance and coordinate AI buildouts."],
    ["Frontier AI", "Prometheus and Google TPU supply-chain moves point to physical-world data and manufacturing as key AI moats."],
    ["Geopolitics", "Meta's Manus split shows how policy pressure can reshape AI acquisitions and operating structures."],
  ],
};
      ...
      <aside className="flex w-64 shrink-0 flex-col border-r border-border bg-card/40">
        ...
        <nav className="mt-2 flex-1 space-y-0.5 px-3">
          {navItems.map(({ label, icon: Icon, view }) => {
            ...
            if (view === "dashboard" || view === "daily-brief") {
              return (
                <button
                  key={label}
                  type="button"
  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-border pb-5">
        <div className="text-sm text-muted-foreground">{dailyBrief.date} · AI Daily Brief newsletter</div>
        <h1 className="mt-2 text-4xl font-semibold tracking-tight">{dailyBrief.title}</h1>
        <p className="mt-3 max-w-5xl text-base leading-relaxed text-muted-foreground">{dailyBrief.thesis}</p>
      </div>
      ...
      <section className="grid grid-cols-[1fr_0.95fr] gap-5">
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <div className="space-y-3">
            {dailyBrief.takeaways.map((takeaway) => (
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          <div className="mb-4 text-xs font-semibold tracking-widest text-muted-foreground">STORY CLUSTERS</div>
          <div className="space-y-4">
            {dailyBrief.stories.map(([category, summary]) => (
              <article key={summary} className="border-b border-border/70 pb-4 last:border-0 last:pb-0">
                <span className="rounded bg-violet-500/20 px-2 py-1 text-[10px] uppercase tracking-wider text-violet-300">
                  {category}
                </span>
